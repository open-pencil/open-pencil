...
```
packages/
  core/              @verso/core — motore (zero dipendenze DOM)
    src/             Grafo scena, renderer, layout, codec, kiwi, tipi
  cli/               @verso/cli — CLI headless per operazioni .fig
    src/commands/    info, tree, find, export, eval, analyze
  mcp/               @verso/mcp — server MCP per strumenti IA
...
src/
  ...
  engine/            Shim di ri-esportazione da @verso/core
  kiwi/              Shim di ri-esportazione da @verso/core
