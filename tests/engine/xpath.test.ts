...
describe('queryByXPath', () => {
  test('finds nodes by type', () => {
    ...
    const results = queryByXPath(graph, '//RECTANGLE')
    expect(results.length).toBe(2)
    expect(results.every((n) => n.type === 'RECTANGLE')).toBe(true)
  })

  test('filters by attribute comparison', () => {
    const { graph, figma } = setup()
    ...
    const results = queryByXPath(graph, '//RECTANGLE[@width < 200]')
    expect(results.length).toBe(1)
    expect(results[0].name).toBe('Small')
  })

  test('handles descendant axis //FRAME//TEXT', () => {
    const { graph, figma } = setup()
    ...
    const results = queryByXPath(graph, '//FRAME//TEXT')
    expect(results.length).toBe(1)
  ...
  test('respects limit option', () => {
    ...
    const results = queryByXPath(graph, '//RECTANGLE', { limit: 3 })
    expect(results.length).toBe(3)
  })

  test('filters by page option', () => {
    const { graph, figma } = setup()
    ...
    const page1Results = queryByXPath(graph, '//RECTANGLE', { page: page1Name })
    expect(page1Results.length).toBe(1)
    expect(page1Results[0].name).toBe('Page1Rect')

    const page2Results = queryByXPath(graph, '//RECTANGLE', { page: 'Page 2' })
  ...
  test('returns empty for no matches', () => {
    ...
    const results = queryByXPath(graph, '//ELLIPSE')
    expect(results.length).toBe(0)
  })

  test('handles invalid selector gracefully (throws)', () => {
    const { graph } = setup()
    expect(() => queryByXPath(graph, '///invalid[[[[')).toThrow()
  })
})

describe('matchByXPath', () => {
  test('returns true for matching node', () => {
    const { graph, figma } = setup()
    ...
    const result = matchByXPath(graph, '@name = "TestRect"', sceneNode)
    expect(result).toBe(true)
  })

  test('returns false for non-matching node', () => {
    const { graph, figma } = setup()
    const rect = figma.createRectangle()
    rect.resize(100, 100)
    rect.name = 'Other'
