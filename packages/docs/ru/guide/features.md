...
**Копирование и вставка с Figma** — выделите узлы в Figma, <kbd>⌘</kbd><kbd>C</kbd>, переключитесь на Inkly, <kbd>⌘</kbd><kbd>V</kbd>. Заливки, обводки, авто-макет, текст, эффекты, радиусы скругления и векторные сети сохраняются. Работает в обоих направлениях.
...
```sh
inkly tree design.fig          # Дерево узлов
inkly find design.fig --type TEXT  # Поиск
inkly export design.fig -f png     # Рендеринг
inkly analyze colors design.fig    # Аудит цветов
inkly analyze clusters design.fig  # Повторяющиеся паттерны
...
```sh
inkly tree                     # Текущий документ
inkly export -f png            # Скриншот канваса
```

Все команды поддерживают `--json`. Установка: `npm install -g @inkly/cli`
...
**Веб** — работает на [app.inkly.dev](https://app.inkly.dev), устанавливается как PWA на мобильных устройствах с адаптированным интерфейсом.
...
Когда шрифт недоступен локально, Inkly автоматически загружает его из Google Fonts. Ручная установка не требуется при открытии файлов .fig с незнакомыми шрифтами.
