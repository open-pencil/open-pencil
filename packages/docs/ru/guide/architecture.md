...
Когда запущено десктоп-приложение, команды CLI подключаются к нему через сервер MCP вместо загрузки файла .fig. На macOS и Linux сервер MCP слушает на Unix-сокете с опциональным TCP на `127.0.0.1:7600` (HTTP + WebSocket). На Windows сервер привязывается только по TCP на `127.0.0.1:7600`. Команды выполняются относительно состояния живого редактора, позволяя скриптам автоматизации и ИИ-агентам взаимодействовать с запущенным приложением.
