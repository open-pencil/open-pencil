...
function toolKind(name: string): acp.ToolKind {
  if (name.startsWith('get_') || name.startsWith('list_') || name === 'node_bounds' || name === 'render')
    return 'read'
  if (name.startsWith('find_')) return 'search'
  if (name === 'delete_node') return 'delete'
  if (name === 'reparent_node' || name === 'node_move') return 'move'
  if (name === 'eval') return 'execute'
  if (
    name.startsWith('set_') ||
    name.startsWith('create_') ||
    name.startsWith('update_') ||
    name.startsWith('clone_') ||
    name.startsWith('rename_') ||
    name.startsWith('group_') ||
    name.startsWith('ungroup_')
  )
    return 'edit'
  return 'other'
}
...
export function createAgent(
  ...
  return {
    async initialize(_params) {
      connection.signal.addEventListener('abort', () => sessions.clear())
      return {
