...
function applyProgress(p: CloudImportProgress) {
  ...
  if (p.phase === 'uploading') {
    ...
    const tickMs = mb >= 10 ? 800 : mb >= 2 ? 400 : 250
    const rate = mb >= 10 ? 0.018 : mb >= 2 ? 0.03 : 0.045
...
async function onRename(id: string, current: string) {
  const next = prompt('Rename canvas', current)
  ...
  >
    ...
    >
      ...
      >
        ...
        <p
          v-if="uploadFileLabel"
          class="max-w-full truncate text-sm text-muted"
    ...
    <header class="flex items-center justify-between border-b border-border px-6 py-4">
      <div>
        <h1 class="text-lg font-semibold tracking-tight">{{ dialogs.cloudHomeTitle }}</h1>
        <p class="text-xs text-muted">OpenPencil</p>
      </div>
    ...
    >
      ...
      <div
        v-else-if="error && canvases.length === 0"
          ...
          >
            ...
            >
              ...
              <span
                ...
                class="absolute right-1.5 top-1.5 rounded bg-black/55 px-1.5 py-0.5 text-[9px] font-medium text-white/90 backdrop-blur-sm"
              >
                {{ syncBadgeLabel(canvas.syncStatus) }}
              </span>
            </div>
