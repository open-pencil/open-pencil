...
- Accept `blur` as the radius in the `dropShadow`, `innerShadow` and blur effect helpers, as the `shadow` shorthand and the `blur` prop already do, and, when rendering JSX, warn about any option the paint and effect helpers ignore instead of dropping it silently (#736).
