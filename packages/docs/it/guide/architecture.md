...
Quando l'app desktop è in esecuzione, i comandi CLI si connettono tramite il server MCP invece di richiedere un file .fig. Su macOS e Linux, il server MCP è in ascolto su un socket di dominio Unix con TCP opzionale su `127.0.0.1:7600` (HTTP + WebSocket). Su Windows, il server si collega solo tramite TCP su `127.0.0.1:7600`. I comandi vengono eseguiti sullo stato dell'editor live, consentendo a script di automazione e agenti AI di interagire con l'app in esecuzione.
