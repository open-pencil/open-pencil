...
const NAME_ROLE_PATTERNS: { pattern: RegExp; role: string; props?: Record<string, unknown> }[] = [
...
function detectRoleFromName(name: string): RoleResult | null {
  const base = getBaseName(name)
  for (const { pattern, role, props } of NAME_ROLE_PATTERNS) {
    if (pattern.test(base)) return { role, props }
  }
  return null
}
...
function describeVisual(node: SceneNode): string {
  ...
  if (node.strokes.length > 0 && node.strokes[0]!.visible !== false) parts.push('bordered')
  ...
  for (const e of node.effects) {
    ...
    else if (e.type === 'BACKGROUND_BLUR') parts.push('backdrop blur')
  }
...
function describeLayout(node: SceneNode): string | null {
  ...
  if (allSame && pad[0]! > 0) parts.push(`${pad[0]}px padding`)
...
function detectIssues(node: SceneNode, gridSize: number): DescribeIssue[] {
  ...
  if (node.type === 'FRAME' || node.type === 'COMPONENT' || node.type === 'INSTANCE') {
    if (node.fills.length === 0 && node.childIds.length === 0) {
...
function describeChild(node: SceneNode): DescribeChild {
  let role = 'generic'
  const nameDetected = detectRoleFromName(node.name)
  if (nameDetected) role = nameDetected.role

  if (role === 'generic' && node.type === 'TEXT') {
    const level = headingLevel(node.fontSize)
    role = level ? `heading(${level})` : 'StaticText'
  }
  if (role === 'generic' && looksLikeSeparator(node)) role = 'separator'
  if (role === 'generic' && looksLikeButton(node)) role = 'button'
  ...
  if (node.type === 'TEXT') {
    ...
    const textColor = node.fills.find((f) => f.type === 'SOLID' && f.visible !== false)
...
export const describe = defineTool({
  name: 'describe',
  description:
    'Semantic description of a node: role, visual style, layout, children summary, and design issues. Complements get_jsx (structural) with intent and warnings.',
  params: {
    id: { type: 'string', description: 'Node ID', required: true },
  ...
  execute: (figma, args) => {
    ...
    let role = 'generic'
    const nameDetected = detectRoleFromName(raw.name)
    if (nameDetected) role = nameDetected.role
    if (role === 'generic' && raw.type === 'TEXT') {
      const level = headingLevel(raw.fontSize)
      role = level ? `heading(${level})` : 'StaticText'
    }
    if (role === 'generic' && looksLikeSeparator(raw)) role = 'separator'
    if (role === 'generic' && looksLikeButton(raw)) role = 'button'
