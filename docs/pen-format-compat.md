...
Inkly 標準実装は限定的に対応していますが、pencil.dev が生成するファイルでは多くの場合レイアウトが崩れます。
このリポジトリでは pencil.dev のレンダリングエンジン仕様をリバースエンジニアリングし、互換性を高めています。
...
→ Inkly 標準パーサーは frame の layout 未指定時を `NONE` 扱いするが、これは誤り。
...
| `.fig` への保存 | `.pen` で開いたファイルは保存時に `.fig` 形式に変換される (Inkly の制約) |
