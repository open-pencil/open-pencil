...
describe('derived instance layout regressions', () => {
  ...
  test.skipIf(isLfsPointer('tests/fixtures/gold-preview.fig'))(
    'preserves repeated badge overrides without moving sibling component wrappers',
    () => {
  ...
  test.skipIf(isLfsPointer('tests/fixtures/gold-preview.fig'))(
    'propagates nested badge component property overrides through cloned instances',
    () => {
      const input = previewChild(layoutGraph, layoutNodes, 'Input')
      const inputRoot = childNamed(layoutGraph, input, '_input')
      const inputFrame = childNamed(layoutGraph, inputRoot, 'Input')
  ...
  test.skipIf(isLfsPointer('tests/fixtures/gold-preview.fig'))(
    'does not collapse unrelated datepicker instances to the page origin',
    () => {
  ...
  test.skipIf(isLfsPointer('tests/fixtures/gold-preview.fig'))(
    'keeps checked-list icons aligned with labels',
    () => {
  ...
  test.skipIf(isLfsPointer('tests/fixtures/gold-preview.fig'))(
    'propagates static icon color overrides through checked-list clones',
    () => {
      const title = previewChild(layoutGraph, layoutNodes, 'Title + Description')
      const checkedList = childNamed(layoutGraph, title, 'Checked List')
      const listItems = checkedList ? layoutGraph.getChildren(checkedList.id) : []
  ...
  test.skipIf(isLfsPointer('tests/fixtures/gold-preview.fig'))(
    'propagates nested component overrides with lazy first-page import',
    async () => {
  ...
  test.skipIf(isLfsPointer('tests/fixtures/gold-preview.fig'))(
    'preserves WYSIWYG toolbar padding',
    () => {
  ...
  test.skipIf(isLfsPointer('tests/fixtures/gold-preview.fig'))(
    'scales WYSIWYG icon vector geometry with nested instances',
    () => {
      const wysiwyg = previewChild(layoutGraph, layoutNodes, '_WYSIWYG-editor')
      const toolbarRoot = childNamed(layoutGraph, wysiwyg, '_on-text-WYSIWYG-toolbar')
      const toolbar = childMatching(
  ...
  test.skipIf(isLfsPointer('tests/fixtures/gold-preview.fig'))(
    'exports WYSIWYG and logo visual overflow instead of clipping to node boxes',
    () => {
      const wysiwyg = previewChild(layoutGraph, layoutNodes, '_WYSIWYG-editor')
