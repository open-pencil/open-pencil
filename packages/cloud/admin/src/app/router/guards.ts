import { CloudAdminAPIError } from '#admin/api/client'
import { queryClient } from '#admin/app/query/client'
import { accountQueryOptions, mfaStatusQueryOptions } from '#admin/app/query/options'
import type { NavigationGuard } from 'vue-router'

import { cloudRedirectPath } from '@open-pencil/cloud/client'

function signInRedirect(path: string) {
  return { name: 'sign-in', query: { redirect: path } }
}

async function activeAccountDestination(path: string) {
  try {
    const account = await queryClient.ensureQueryData(accountQueryOptions())
    if (account.state === 'active') return true
    return { name: `account-${account.state}`, query: { redirect: cloudRedirectPath(path) } }
  } catch (error) {
    if (error instanceof CloudAdminAPIError && error.kind === 'authentication-required') {
      return signInRedirect(path)
    }
    throw error
  }
}

export const resolveAnonymous: NavigationGuard = async (to) => {
  try {
    const account = await queryClient.ensureQueryData(accountQueryOptions())
    if (account.state === 'active') return cloudRedirectPath(to.query.redirect)
    return {
      name: `account-${account.state}`,
      query: { redirect: cloudRedirectPath(to.query.redirect) }
    }
  } catch (error) {
    if (error instanceof CloudAdminAPIError && error.kind === 'authentication-required') return true
    throw error
  }
}

export function requireAccountState(
  expectedState: 'pending' | 'rejected' | 'revoked'
): NavigationGuard {
  return async (to) => {
    try {
      const account = await queryClient.ensureQueryData(accountQueryOptions())
      if (account.state === expectedState) return true
      if (account.state === 'active') return cloudRedirectPath(to.query.redirect)
      return {
        name: `account-${account.state}`,
        query: { redirect: cloudRedirectPath(to.query.redirect) }
      }
    } catch (error) {
      if (error instanceof CloudAdminAPIError && error.kind === 'authentication-required') {
        return signInRedirect(cloudRedirectPath(to.query.redirect))
      }
      throw error
    }
  }
}

export const requireActiveAccount: NavigationGuard = (to) => activeAccountDestination(to.fullPath)

export const requireDeploymentAdmin: NavigationGuard = async (to) => {
  const active = await activeAccountDestination(to.fullPath)
  if (active !== true) return active
  const account = await queryClient.ensureQueryData(accountQueryOptions())
  if (account.user.deploymentRole !== 'admin') return { name: 'admin-forbidden' }
  const mfa = await queryClient.ensureQueryData(mfaStatusQueryOptions())
  if (!mfa.mfa.enabled) {
    return { name: 'account-security', query: { setup: 'mfa', redirect: to.fullPath } }
  }
  return mfa.mfa.required ? { name: 'mfa-challenge', query: { redirect: to.fullPath } } : true
}
