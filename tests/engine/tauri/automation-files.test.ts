...
describe('Tauri automation file handling', () => {
  test('creates parent directory for automation new-document paths', async () => {
    ...
    await mockTauriIPC((cmd, args) => {
      calls.push({ cmd, args })
      if (cmd === 'plugin:path|dirname') return '/tmp/inkly/nested'
      return null
    })

    await ensureTauriParentDirectory('/tmp/inkly/nested/file.fig')

    expect(calls).toEqual([
      {
        cmd: 'plugin:path|dirname',
        args: { path: '/tmp/inkly/nested/file.fig' }
      },
      {
        cmd: 'plugin:fs|mkdir',
        args: { path: '/tmp/inkly/nested', options: { recursive: true } }
      }
    ])
  })
