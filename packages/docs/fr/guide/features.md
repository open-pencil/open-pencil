...
**Copier-coller avec Figma** — sélectionnez des nœuds dans Figma, <kbd>⌘</kbd><kbd>C</kbd>, passez à Inkly, <kbd>⌘</kbd><kbd>V</kbd>. Les remplissages, contours, auto-layout, texte, effets, rayons de coins et réseaux vectoriels sont préservés. Fonctionne dans les deux sens.
...
CLI : `inkly export design.fig -f jsx --style tailwind`
...
```sh
inkly tree design.fig          # Arbre de nœuds
inkly find design.fig --type TEXT  # Recherche
inkly export design.fig -f png     # Rendu
inkly analyze colors design.fig    # Audit des couleurs
inkly analyze clusters design.fig  # Motifs répétés
inkly eval design.fig -c "..."     # API Plugin Figma
```

Lorsque l'application de bureau est lancée, omettez le fichier pour contrôler l'éditeur en direct via RPC :

```sh
inkly tree                     # Document en direct
inkly export -f png            # Capture du canevas
```

Toutes les commandes supportent `--json`. Installation : `npm install -g @inkly/cli`
...
**Web** — disponible sur [app.inkly.dev](https://app.inkly.dev), installable en PWA sur mobile avec interface tactile optimisée.
...
Lorsqu'une police n'est pas disponible localement, Inkly la récupère automatiquement depuis Google Fonts. Aucune installation manuelle nécessaire lors de l'ouverture de fichiers .fig avec des polices inconnues.
