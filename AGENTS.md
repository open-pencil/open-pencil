...
- `packages/cloud` — `@open-pencil/cloud`: portable Cloud contracts, client discovery, and Hono server application foundation. Keep deployment runtimes, auth, database, object storage, and collaboration adapters behind explicit package subpaths so browser consumers do not pull server dependencies. Cloud architecture, configuration ownership, and unresolved runtime-policy questions live in `packages/cloud/README.md`; deployment-specific instructions remain under `packages/cloud/deploy/**`.
...
- `@open-pencil/kiwi` — low-level Kiwi/FIG schema-runtime, codec, container, GUID, and parse helpers.
...
- `@open-pencil/cloud/runtime/cloudflare` — Cloudflare Workers/Hyperdrive assembly with request-scoped PostgreSQL resources and scheduled cleanup.
...
- Collaboration lives under `src/app/collab/**` and is documented in `packages/docs/programmable/collaboration.md`. It uses Trystero + Yjs + awareness; preserve crypto-safe room IDs and peer cleanup semantics when changing it. Cloud documents join ticket-derived, password-protected rooms, publish authenticated or stable guest identities with view/edit presence, and refresh short-lived tickets before expiry. Keep `serverEnforcedWrites: false` explicit until an authenticated relay exists; P2P tickets do not provide authoritative server-side write enforcement.
