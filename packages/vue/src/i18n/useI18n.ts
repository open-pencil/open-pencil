import { useStore } from '@nanostores/vue'
import type { Ref } from 'vue'

import { locale, setLocale, AVAILABLE_LOCALES, LOCALE_LABELS } from '#vue/i18n/locale'
import type { Locale } from '#vue/i18n/locale'
import {
  menuMessages,
  commandMessages,
  toolMessages,
  panelMessages,
  variableTypeMessages,
  pageMessages,
  dialogMessages,
  boardsMessages,
  dashboardMessages,
  accountMessages,
  boardSettingsMessages,
  notificationsMessages,
  notificationsFormatMessages,
  notificationBellMessages,
  boardCardMessages,
  teamCardMessages,
  loginBannerMessages,
  mobilePresenceMessages,
  mentionInputMessages,
  fontSettingsMessages,
  safariBannerMessages,
  shareModalMessages,
  teamsMessages,
  teamDetailMessages,
  teamSettingsMessages,
  commonMessages,
  inviteAuthMessages,
  saveAndLeaveModalMessages,
  adminMessages
} from '#vue/i18n/messages'

/**
 * Reactive i18n composable for Inkly Vue components.
 *
 * Returns reactive translation objects grouped by domain, plus locale
 * controls. All values update automatically when the locale changes.
 *
 * @example
 * ```vue
 * <script setup>
 * const { menu, commands, locale, setLocale } = useI18n()
 * </script>
 *
 * <template>
 *   <button>{{ menu.save }}</button>
 *   <span>{{ commands.undo }}</span>
 * </template>
 * ```
 */
export function useI18n() {
  return {
    menu: useStore(menuMessages),
    commands: useStore(commandMessages),
    tools: useStore(toolMessages),
    panels: useStore(panelMessages),
    variableTypes: useStore(variableTypeMessages),
    pages: useStore(pageMessages),
    dialogs: useStore(dialogMessages),
    boards: useStore(boardsMessages),
    dashboard: useStore(dashboardMessages),
    account: useStore(accountMessages),
    boardSettings: useStore(boardSettingsMessages),
    notifications: useStore(notificationsMessages),
    notificationsFormat: useStore(notificationsFormatMessages),
    notificationBell: useStore(notificationBellMessages),
    boardCard: useStore(boardCardMessages),
    teamCard: useStore(teamCardMessages),
    loginBanner: useStore(loginBannerMessages),
    mobilePresence: useStore(mobilePresenceMessages),
    mentionInput: useStore(mentionInputMessages),
    fontSettings: useStore(fontSettingsMessages),
    safariBanner: useStore(safariBannerMessages),
    shareModal: useStore(shareModalMessages),
    teams: useStore(teamsMessages),
    teamDetail: useStore(teamDetailMessages),
    teamSettings: useStore(teamSettingsMessages),
    common: useStore(commonMessages),
    inviteAuth: useStore(inviteAuthMessages),
    saveAndLeaveModal: useStore(saveAndLeaveModalMessages),
    admin: useStore(adminMessages),
    locale: useStore(locale) as Ref<Locale>,
    availableLocales: AVAILABLE_LOCALES,
    localeLabels: LOCALE_LABELS,
    setLocale
  }
}
