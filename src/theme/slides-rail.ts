const slidesRailTheme = {
  slots: {
    ...
    cell: 'flex w-full cursor-pointer items-start gap-1.5 rounded-lg border-0 bg-transparent p-0 text-left outline-none focus-visible:ring-1 focus-visible:ring-panel-focus',
    index:
      'mt-1 w-4 shrink-0 text-right text-[11px] font-medium tabular-nums text-muted data-[active=true]:text-accent',
    // Width clamped via inline style (min/max) so thumbs scale with the left panel
    thumbShell: 'min-w-0 shrink grow',
    activeChrome:
      'rounded-lg p-1 data-[active=true]:bg-accent data-[active=false]:bg-transparent data-[active=false]:p-0',
    thumb:
