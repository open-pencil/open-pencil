...
```
packages/
  core/              @inkly/core — motore (zero dipendenze DOM)
    src/             Grafo scena, renderer, layout, codec, kiwi, tipi
  cli/               @inkly/cli — CLI headless per operazioni .fig
    src/commands/    info, tree, find, export, eval, analyze
  mcp/               @inkly/mcp — server MCP per strumenti IA
...
src/
  ...
  engine/            Shim di ri-esportazione da @inkly/core
  kiwi/              Shim di ri-esportazione da @inkly/core
...
Sviluppatori e agenti IA dovrebbero leggere `AGENTS.md` nella root del repo ([vedi su GitHub](https://github.com/cardene777/open-pencil/blob/master/AGENTS.md)). Copre rendering, grafo scena, componenti e istanze, layout, UI, formato file, convenzioni Tauri e problemi noti.
