...
describe('gold-preview.fig clipboard roundtrip', () => {
  ...
  it.skipIf(isLfsPointer('tests/fixtures/gold-preview.fig'))(
    'OpenPencil format: zero property differences',
    () => {
  ...
  it.skipIf(isLfsPointer('tests/fixtures/gold-preview.fig'))(
    'OpenPencil format: compressed data is under 1MB',
    () => {
      const html = buildOpenPencilClipboardHTML(topLevelNodes, graph)
  ...
  it.skipIf(isLfsPointer('tests/fixtures/gold-preview.fig'))(
    'Figma format: preserves clipsContent, constraints, arcData, layoutAlignSelf for matched nodes',
    async () => {
      ...
      for (const o of origAll) {
        ...
        if ((p.arcData != null) !== (o.arcData != null))
          errors.push(
            `arcData: "${o.name}" expected ${o.arcData != null}, got ${p.arcData != null}`
          )
      }
