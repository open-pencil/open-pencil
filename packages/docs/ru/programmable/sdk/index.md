---
title: Vue SDK
description: Создавайте редакторы на базе Inkly с помощью headless-компосаблов и примитивов для Vue.
---

# Vue SDK

`@inkly/vue` существует для того, чтобы Inkly мог быть чем-то большим, чем самостоятельное дизайн-приложение.

Цель — сделать Inkly набором инструментов, который можно встроить в другие продукты, внутренние инструменты и редакторы под конкретные задачи, а не только использовать в виде единого стандартного интерфейса.

Приложение Inkly — одна из возможных компоновок этого набора. SDK — это то, с помощью чего вы создаёте другие.
...
- **Интеграция с фреймворком**: Vue-интеграция поверх `@inkly/core`
