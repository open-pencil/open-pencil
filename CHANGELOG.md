...
- Add a reference OpenPencil Cloud Compose deployment with PostgreSQL and SeaweedFS, plus a lightweight Garage compatibility profile, including automatic migrations, readiness checks, persistent storage, full revision-flow integration coverage, bounded multi-worker cleanup, resilient concurrent multipart uploads, and a deployment-independent typed API router.
