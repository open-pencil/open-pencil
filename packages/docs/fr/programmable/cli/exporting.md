...
```sh
inkly export design.fig                          # PNG (par défaut)
inkly export design.fig -f jpg -s 2 -q 90       # JPG en 2×, qualité 90
inkly export design.fig -f webp -s 3             # WEBP en 3×
inkly export design.fig -f svg                   # SVG vectoriel
```
...
Supporte aussi `--style inkly` pour le format JSX natif (voir [Moteur de rendu JSX](../jsx-renderer)).
...
```sh
inkly export -f png    # capture du canevas actuel
```
